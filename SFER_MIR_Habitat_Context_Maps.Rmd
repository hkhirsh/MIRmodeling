...
- **Batch C2 — MIR sites (expanded context)**  
...
if (regen_maps) {
  ...
  # 👇 CHANGE THIS to the run folder that already exists on your machine
  # run_dir <- "Outputs_sfer_habitat_maps/run_20260205_1356"
    # run_dir <- "docs/Outputs_sfer_habitat_maps/run_SHARE"
  run_dir <- file.path("Outputs_sfer_habitat_maps", "run_SHARE")
...
# DISPLAY ONLY (no sf / no ggsave) — uses the PNG already in your run folder
ref_png_fs  <- file.path(run_dir, "Figures", "reference_map_static_keys.png")
ref_png_web <- to_web(ref_png_fs)

if (!file.exists(ref_png_fs)) {
  cat("<p><b>Reference map not found:</b> ", ref_png_web, "</p>", sep = "")
} else {
  cat(sprintf(
...
#     else NULL } +
# 
#   # MIR labels — leader lines OK, pushed farther away
...
#     else NULL } +
# 
#   # SFER points — default ggplot colors by Zone (no manual palette)
#   geom_sf(
#     data = sfer_keys,
...
#   labs(
#     fill  = "SFER Zone",
#     title = "SFER stations and MIR sites — Florida Keys"
#   )
# 
...
## Map Outputs
<details><summary><strong>SFER — scale 30 km (`r length(list.files(file.path(run_dir,"Figures","SFER_context30_hab50"), pattern="\\.png$"))` maps)</strong></summary>
...
<details><summary><strong>SFER — scale 10 km (`r length(list.files(file.path(run_dir,"Figures","SFER_context10_hab25_zoom"), pattern="\\.png$"))` maps)</strong></summary>
```{r, echo=FALSE, results='asis', comment=NA}
...
<details><summary><strong>MIR — scale 10 km (`r length(list.files(file.path(run_dir,"Figures","MIR_context10_hab10"), pattern="\\.png$"))` maps)</strong></summary>
```{r, echo=FALSE, results='asis', comment=NA}
...
<details><summary><strong>MIR — scale 5 km (`r length(list.files(file.path(run_dir,"Figures","MIR_context5_hab10"), pattern="\\.png$"))` maps)</strong></summary>
```{r, echo=FALSE, results='asis', comment=NA}
  ...
  DT::datatable(
    ...
    options = list(
      ...
      initComplete = DT::JS(
        ...
        "  cell.empty();",
        "  var select = $('<select style=\"width: 100%\"><option value=\"\">Select MIR site…</option></select>')",
        "    .appendTo(cell)",
        "    .on('change', function(){",
